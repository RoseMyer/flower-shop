    ...
    <Container maxWidth="lg" sx={{py:3}}>
      <Stack direction="column" spacing={3} sx={{ width: '100%', alignItems: 'center', justifyContent: 'center', my: 7.5 }}>
        ...
        {items && Object.keys(items).length > 0 ? 
          Object.values(items).map(cartItem => (<CartItemCard key={cartItem.item.id} cartItem={cartItem} />)) : 
          <Typography variant="h4" sx={{ textAlign: 'center', color: 'white' }}>Your cart is empty!</Typography>}

        {items && Object.keys(items).length > 0 ? 
          <CheckoutButton /> : 
          <Button variant="contained" color="primary" onClick={backToProducts} sx={{ mt: 2 }}>
            Back to Products
          </Button>}
      </Stack>
    </Container>
